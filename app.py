...
st.markdown("""
    <style>
    /* AJUSTE PARA DEIXAR OS CARDS MENORES */
    .status-card {
        padding: 10px 15px;
        border-radius: 8px;
        text-align: center;
    ...
    .card-azul { background-color: #1C83E1; color: white; border-bottom: 4px solid #1461a8; }
...
if not df_raw.empty:
    ...
    with tab2:
        ...
        # Filtros encadeados de forma segura (um filtra o outro estritamente)
        ...
        with col_d:
            ...
            if f2_ped != "TODOS":
                ...
                # Corrigindo a porcentagem no gráfico de pizza
                st.markdown(f"##### Volume Geral de Entregas")
                df_pie_sla = df2_b['PERFORMANCE_SLA'].value_counts().reset_index()
    ...
    with tab3:
        ...
        df_rank_cards = df3.groupby('TRANSPORTADORA').apply(calc_otd).reset_index()
        df_rank_cards.columns = ['TRANSPORTADORA','OTD']
        df_rank_cards = df_rank_cards[df_rank_cards['OTD'] >= 0]
        df_rank_cards['CLASS'] = df_rank_cards['OTD'].apply(lambda x: get_otd_info(x)[0])

        if f3_sla_regua != "TODOS": 
            df_rank_cards = df_rank_cards[df_rank_cards['CLASS'] == f3_sla_regua]
            df3 = df3[df3['TRANSPORTADORA'].isin(df_rank_cards['TRANSPORTADORA'])]

        n_exc = len(df_rank_cards[df_rank_cards['CLASS'] == 'EXCELENTE'])
        n_boa = len(df_rank_cards[df_rank_cards['CLASS'] == 'BOA'])
        n_ale = len(df_rank_cards[df_rank_cards['CLASS'] == 'ALERTA'])
        n_cri = len(df_rank_cards[df_rank_cards['CLASS'] == 'CRÍTICA'])

        st.write("")
        c3a, c3b, c3c, c3d = st.columns(4)
        c3a.markdown(f"<div class='status-card card-verde'><h3>🟢 EXCELENTE</h3><h1>{n_exc}</h1></div>", unsafe_allow_html=True)
        c3b.markdown(f"<div class='status-card card-azul'><h3>🔵 BOA</h3><h1>{n_boa}</h1></div>", unsafe_allow_html=True)
        c3c.markdown(f"<div class='status-card card-amarelo'><h3>🟡 ALERTA</h3><h1>{n_ale}</h1></div>", unsafe_allow_html=True)
        c3d.markdown(f"<div class='status-card card-vermelho'><h3>🔴 CRÍTICA</h3><h1>{n_cri}</h1></div>", unsafe_allow_html=True)
        st.write("")
