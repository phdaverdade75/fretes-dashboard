...
st.markdown("""
    <style>
    /* Estilos Adicionados para os Cards da Página 2 e 3 */
    .status-card {
        padding: 15px;
        border-radius: 10px;
        text-align: center;
    ...
    .card-verde { background-color: #00C49F; color: white; border-bottom: 4px solid #008f73; }
    .card-vermelho { background-color: #FF4B4B; color: white; border-bottom: 4px solid #b33434; }
    .card-amarelo { background-color: #FFA500; color: white; border-bottom: 4px solid #cc8400; }
    ...
    if not df.empty:
        ...
        with col_conteudo:
            ...
            with cg1:
                ...
                # ADIÇÃO: text='VLR DO FRETE' e formatação de R$
...
with tab2:
    if not df.empty:
        ...
        # --- ADIÇÃO: OS 3 CARDS DINÂMICOS DA PÁGINA 2 ---
        n_prazo_t2 = len(df_t2[df_t2['PERFORMANCE_SLA'] == 'NO PRAZO'])
        n_atraso_t2 = len(df_t2[df_t2['PERFORMANCE_SLA'] == 'ATRASADO'])
        n_andamento_t2 = len(df_t2[df_t2['PERFORMANCE_SLA'] == 'EM ANDAMENTO'])

        c_prazo_t2, c_atraso_t2, c_andamento_t2 = st.columns(3)
        c_prazo_t2.markdown(f"<div class='status-card card-verde'><h3>✅ NO PRAZO</h3><h1>{n_prazo_t2}</h1></div>", unsafe_allow_html=True)
        c_atraso_t2.markdown(f"<div class='status-card card-vermelho'><h3>🚨 ATRASADO</h3><h1>{n_atraso_t2}</h1></div>", unsafe_allow_html=True)
        c_andamento_t2.markdown(f"<div class='status-card card-amarelo'><h3>⏳ EM ANDAMENTO</h3><h1>{n_andamento_t2}</h1></div>", unsafe_allow_html=True)
        st.write("")
        # -----------------------------------------------
...
with tab3:
    if not df.empty:
        ...
        # --- ADIÇÃO: OS 3 CARDS DINÂMICOS DA PÁGINA 3 ---
        n_prazo_t3 = len(df_t3[df_t3['PERFORMANCE_SLA'] == 'NO PRAZO'])
        n_atraso_t3 = len(df_t3[df_t3['PERFORMANCE_SLA'] == 'ATRASADO'])
        n_andamento_t3 = len(df_t3[df_t3['PERFORMANCE_SLA'] == 'EM ANDAMENTO'])

        c_prazo_t3, c_atraso_t3, c_andamento_t3 = st.columns(3)
        c_prazo_t3.markdown(f"<div class='status-card card-verde'><h3>✅ NO PRAZO</h3><h1>{n_prazo_t3}</h1></div>", unsafe_allow_html=True)
        c_atraso_t3.markdown(f"<div class='status-card card-vermelho'><h3>🚨 ATRASADO</h3><h1>{n_atraso_t3}</h1></div>", unsafe_allow_html=True)
        c_andamento_t3.markdown(f"<div class='status-card card-amarelo'><h3>⏳ EM ANDAMENTO</h3><h1>{n_andamento_t3}</h1></div>", unsafe_allow_html=True)
        st.write("")
        # -----------------------------------------------
