...
st.markdown("""
    ...
    .b-vermelho { border-left: 4px solid #FF4B4B; }
    .t-vermelho { color: #FF4B4B; }
    
    .b-amarelo { border-left: 4px solid #FFA500; }
    .t-amarelo { color: #FFA500; }
    
    .b-azul { border-left: 4px solid #1C83E1; }
    .t-azul { color: #1C83E1; }
    </style>
...
# ==========================================
# PÁGINA 2: MAPA DE CALOR E SLA LOGÍSTICO
# ==========================================
with tab2:
    if not df.empty:
...
with tab3:
    if not df.empty:
        ...
        r_sla_cards = df_t3.groupby('TRANSPORTADORA').apply(calc_otd_global_cards).reset_index()
        r_sla_cards.columns = ['TRANSPORTADORA', 'OTD_PERCENTUAL']
        r_sla_cards = r_sla_cards[r_sla_cards['OTD_PERCENTUAL'] >= 0] 
        r_sla_cards['CLASSIFICAÇÃO'] = r_sla_cards['OTD_PERCENTUAL'].apply(lambda x: get_otd_info(x)[0])
        
        n_exc = len(r_sla_cards[r_sla_cards['CLASSIFICAÇÃO'] == 'EXCELENTE'])
        n_boa = len(r_sla_cards[r_sla_cards['CLASSIFICAÇÃO'] == 'BOA'])
        n_ale = len(r_sla_cards[r_sla_cards['CLASSIFICAÇÃO'] == 'ALERTA'])
        n_cri = len(r_sla_cards[r_sla_cards['CLASSIFICAÇÃO'] == 'CRÍTICA'])
        
        st.write("")
        c3a, c3b, c3c, c3d = st.columns(4)
        c3a.markdown(f"<div class='card-mini b-verde'><h4>🟢 EXCELENTE</h4><h2 class='t-verde'>{n_exc}</h2></div>", unsafe_allow_html=True)
        c3b.markdown(f"<div class='card-mini b-azul'><h4>🔵 BOA</h4><h2 class='t-azul'>{n_boa}</h2></div>", unsafe_allow_html=True)
